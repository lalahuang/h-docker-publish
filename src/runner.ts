...
export async function runner(publicKey: string, cwd = process.cwd()) {
  ...
  // 
  const publishSettingInfo = storage.settingInfos?.find(item => item.publishKey == publicKey);
  if (!publishSettingInfo)
    return;

  await execaCommand(`${agent} run ${publishSettingInfo.buildCommand}`, { stdio: 'inherit', encoding: 'utf-8', cwd: publishSettingInfo.projectPath })
  await execaCommand(`docker build --pull --rm -f Dockerfile -t ${publishSettingInfo.mirrorImage} ./ `, { stdio: 'inherit', encoding: 'utf-8', cwd: publishSettingInfo.projectPath })
  await execaCommand(`docker push ${publishSettingInfo.mirrorImage}`, { stdio: 'inherit', encoding: 'utf-8', cwd: publishSettingInfo.projectPath })
}
